...
10. When configuring the external terminal viewer launcher, prefer `terminal_viewer_shortcut` with `ctrl+alt+/` by default; it supports `ctrl+<letter>`, `ctrl+shift+<letter>`, `ctrl+alt+<letter>`, `ctrl+,`, `ctrl+shift+,`, `ctrl+alt+,`, or `ctrl+alt+/` and launches the same path as `/subagents-terminal`. Configurable shortcuts can collide with other extensions; `ctrl+alt+,` is accepted for intentional legacy configs but may conflict with extensions such as `pi-powerline-footer`.
