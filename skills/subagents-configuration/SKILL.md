...
10. When configuring the external terminal viewer launcher, prefer `terminal_viewer_shortcut` with `ctrl+alt+,` by default; it supports `ctrl+<letter>`, `ctrl+shift+<letter>`, `ctrl+alt+<letter>`, `ctrl+,`, `ctrl+shift+,`, or `ctrl+alt+,` and launches the same path as `/subagents-terminal`.
