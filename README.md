...
| `terminal_viewer_shortcut` | `ctrl+alt+,` | Shortcut used to launch the same Kitty terminal viewer path as `/subagents-terminal`. Accepts `ctrl+<letter>`, `ctrl+shift+<letter>`, `ctrl+alt+<letter>`, `ctrl+,`, `ctrl+shift+,`, or `ctrl+alt+,`, and also accepts camelCase `terminalViewerShortcut`. |
