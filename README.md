...
- Kitty-first external read-only terminal viewer shell via `/subagents-terminal` or `ctrl+alt+/` by default.
...
```json
{
  ...
  "terminal_viewer_shortcut": "ctrl+alt+/",
...
| `terminal_viewer_shortcut` | `ctrl+alt+/` | Shortcut used to launch the same Kitty terminal viewer path as `/subagents-terminal`. Accepts `ctrl+<letter>`, `ctrl+shift+<letter>`, `ctrl+alt+<letter>`, `ctrl+,`, `ctrl+shift+,`, `ctrl+alt+,`, or `ctrl+alt+/`, and also accepts camelCase `terminalViewerShortcut`. Configurable shortcuts can collide with other extensions; `ctrl+alt+,` is accepted for intentional legacy configs but may conflict with extensions such as `pi-powerline-footer`. |
...
| `ctrl+alt+/` | Open the Kitty terminal viewer by default through the same path as `/subagents-terminal`. Configurable via `terminal_viewer_shortcut` in `subagents.json`. |
