/**
 * Session search/sort logic — extracted verbatim from
 * `packages/coding-agent/src/modes/interactive/components/session-selector-search.ts`.
 *
 * Only the `fuzzyMatch` import is repointed to the local port (`./fuzzy-match.js`)
 * and `SessionInfo` to the local type (`./session-info.js`), so the extension does
 * not depend on Pi internals or `@earendil-works/pi-tui` directly.
 */
